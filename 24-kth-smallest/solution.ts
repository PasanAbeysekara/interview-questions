...
class Solution {
    ...
    findKthSmallest(nums: number[], k: number): number {
        const quickselect = (left: number, right: number, kSmallest: number): number => {
        ...
        const partition = (left: number, right: number, pivotIdx: number): number => {
    ...
    findKthSmallestSort(nums: number[], k: number): number {
        nums.sort((a, b) => a - b);
        return nums[k - 1];
    }
}
